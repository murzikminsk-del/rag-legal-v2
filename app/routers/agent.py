"""Ручки агентного слоя: персистентный ReAct-граф с HIL.
...
async def agent_chat(req: AgentChatRequest, graph: AgentGraphDep) -> AgentChatResponse:
    ...
    result = await graph.ainvoke(
        ...
        _config(req.thread_id, req.user_role),
    )
...
async def agent_resume(req: AgentResumeRequest, graph: AgentGraphDep) -> AgentChatResponse:
    if graph is None:
        raise HTTPException(status_code=503, detail="агентный граф не инициализирован")
    result = await graph.ainvoke(
...
async def agent_stream(req: AgentStreamRequest, graph: AgentGraphDep) -> StreamingResponse:
    ...
    config = _config(req.thread_id, req.user_role)
