...
Опасный инструмент `send_email` проходит через два узла:
- `prepare_email` — idempotent: рендерит payload из tool_call, без side-effect
- `confirm_and_send` — interrupt() перед отправкой, реальная отправка ТОЛЬКО после
  Command(resume=...). При роли `full` interrupt пропускается.

Бэкенд чекпоинтера: AGENT_CHECKPOINTER = memory | sqlite | postgres.
"""
...
def send_email(to: str, subject: str, body: str) -> str:
    ...
    """
    # Граф перехватывает вызов и проводит его через HIL-гейт.
    # Реальная отправка — только после resume.
    ...
    async def execute_tool(state: PersistentAgentState) -> dict:
        ...
        for call in last.tool_calls:
            if call["name"] == DANGEROUS_TOOL:
                continue  # опасный инструмент идёт через HIL-ветку
...
def _psycopg_uri(database_url: str) -> str:
    """AsyncPostgresSaver работает на psycopg v3: без +asyncpg."""
